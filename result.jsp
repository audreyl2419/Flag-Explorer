<%--
  ...
  HTML layout adopted from css template provided by W3SHOOL: https://www.w3schools.com/html/tryit.asp?filename=tryhtml_layout_float
--%>
...
<style>
	* {
	...
	article {
		...
		height: unset; /* only for demonstration, should be removed */
		font-family: Times;
	}
...
<body>
    ...
    <% if (request.getAttribute("data") == null) { %>
        <header><h1>No information found. Please select another country to view.</h1></header>
    <% } else { %>
		<% String[] output = (String[])request.getAttribute("data");%>
		<header><h1><%=output[3]%></h1></header>
        <% if (request.getAttribute("size") == "M") { %>
			<img src="<%=output[1]%>" style="max-width:200px">
        <% } else {%>
            <img src="<%=output[1]%>">
        <% } %>
		<article>
        <p><strong>About the country: </strong> <%=output[2].trim().replace("<p></p>", "/n")%>  </p>
		<p>Credit Source: <ins><%=output[0]%></ins></p>
		</article>
    <% } %><br><br>
	</section>
    <footer>Select anther country to view:
